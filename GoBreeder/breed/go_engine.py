"""Go engine: communicates with external Go programs via GTP2.

Maintains board state and relays moves between the Mediator and an external
Go engine process.
"""
import logging
import sys
...
class GoEngine:
    ...
    def clear_board(self):
        self.board = dict()
        for x in range(0, config.board_size):
            for y in range(0, config.board_size):
    ...
    def initialise(self):  # Do inital handshake
        ...
        for line in handshake_conversation + board_setup_conversation:
            ...
            if reading:
                ...
                if target_verb != verb:
                    self.log(f"ERR: Got :{verb}: Expected :{target_verb}:\n")
                    return False
            else:
                reply = " ".join(tokens[1:])
    ...
    def log(self, msg: str) -> None:
        ...
        if self.logging:
            logger.debug("ENGINE: %s", msg.rstrip())

    visual_pieces = {-1: "B", 0: "-", 1: "W"}
    ...
    def play_pass_and_end(self):
        ...
        verb = self.receive()
        self.log(f"go_engine got verb {verb}\n")
        if "genmove" not in verb:
            self.log(f"go_engine playing pass and end - expected genmove verb got {verb}\n")
        self.send("= PASS\n\n")
