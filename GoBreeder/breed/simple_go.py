    ...
    def legal_move(self, move):
        ...
        for pos in self.iterate_neighbour(move):
            if self.goban[pos] == EMPTY:
                return True
            if self.goban[pos] == self.side and self.liberties(pos) > 1:
                return True
...
def main():
    ...
    while True:
        ...
        if len(g.move_history) >= 2 and g.move_history[-1] == PASS_MOVE and g.move_history[-2] == PASS_MOVE:
            break
