"""Mediator: manages the Breeder-GoEngine connection for GTP-based breeding runs."""
import argparse
import ast
import datetime
import logging
...
class Mediator:
    ...
    def __init__(
        self, population_size=1000, govm=None, go_eng=None, genome=None, gtp_breed=False, silent=False, logprefix=""
    ):
        ...
        while play_games:
            ...
            while game_on:
                ...
                if "PASS" in verb:
                    ...
                    move, pc_history = self.govm.get_move(
                        board=self.go_eng.board, player=self.go_eng.player, program=self.genome
                    )
                    self.move_number += 1
                    if config.record_stats:
                    ...
                    if config.graph_move_pc:
                        pch_df = pd.DataFrame(
                            zip(pc_history, range(0, len(pc_history))),
                            columns=["pc", "idx"],
                            index=range(0, len(pc_history)),
                        )
                        fig = px.line(pch_df, x="idx", y="pc", title=f"PC history for move {self.move_number}")
                        fig.show()
                    ...
                    if result != "ok":
                        self.log(f"ERR - Got {result} back from go_eng play move, expected ok")
                        sys.exit(99)
                    ...
                    self.log(f"ERR - DONT KNOW WHAT TO DO got {verb}")
                    sys.exit(99)
    ...
    def log(self, msg: str) -> None:
        ...
        if self.logging:
            logger.debug("MEDIATOR%s: %s", self.logprefix, msg.rstrip())
...
def setup_and_parse_args():
    ...
    argparser.add_argument(
        "-genome_file", type=str, nargs=1, help="Without -gtp-breed, run a single game with -genome_file"
    )

    argparser.add_argument(
        "-gtp_breed", action="store_true", help="Do a breeding run, using genome_file as starting population"
    )
...
if __name__ == "__main__":
    ...
    setup_logging(_os.path.normpath(_log_dir))
    ...
    if args.genome_file is not None:
        if len(args.genome_file[0]) > 0:
            ...
            if not breed:
                genome_file = open(genome_filename)
                dna = ast.literal_eval(genome_file.readline())  # safe alternative to eval()
                genome_file.close()
